      ...
      <div>
        ...
        <div className="lg:p-[4rem_6rem] max-[1023px]:p-[1.5rem_15px_4px]">
          <ul className="list-decimal px-[20px]">
            {faqDatat.map(item => (
              <li className="pb-[10px] text-start marker:text-[#01959a] marker:font-medium md:text-[1.5rem]"
                key={item.id}>
                <h3 className="text-start md:text-[1.5rem] font-medium text-[#01959aaf] cursor-pointer inline"
                  onClick={() => handleOpenAndClose(item.id)}>
                  {item.question}
                  <i className={isOpen === item.id ? "fa-solid fa-chevron-up" : "fa-solid fa-chevron-down"}></i>
                </h3>
                {isOpen === item.id && 
                  <p className="text-start pb-4 px-[15px] text-[1rem] text-[#01959a] border-b-[2px] border-[#01959a]">
                  {item.answer}
                </p>
                }
              </li>
